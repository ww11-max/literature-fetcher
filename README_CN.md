...
一个免费开源的学术文献元数据批量获取工具。告诉它你要搜哪些期刊、哪些关键词、哪几年，它自动搜索所有组合、去重，输出带标题、作者、DOI 和其他信息的文献表。

**本工具支持多种主流 AI 编程平台，包括 Claude Code、Windsurf / Codex、OpenClaw 以及 Cursor，只需简单配置即可在 AI 助手中一键调用。**

无需 API 密钥，不需要逐个访问期刊官网，大部分检索一分钟内完成。

**输出格式：** Excel (.xlsx)、Markdown (.md)，可选关键词推荐。
...
# 得到：我的检索结果.xlsx、我的检索结果.md、关键词推荐
```
...
本工具可作为 **MCP 工具** 安装到你的 AI 编程助手中。装好后，你只需要用自然语言告诉 AI "帮我搜文献"，它就会自动执行——**不需要记路径，不需要记复杂命令**。
...
在 Windsurf 中打开 `���置 → MCP Servers → 添加 MCP Server`：
